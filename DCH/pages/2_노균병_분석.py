...
import io
import pandas as pd
from utils import calculate_disease_area_ratio # utils.py에서 함수 임포트
from streamlit_cropper import st_cropper # streamlit-cropper 임포트

st.set_page_config(page_title="노균병 분석", layout="wide")
st.title("🟡 노균병 분석")
st.markdown("노균병 의심 오이 잎 사진을 업로드하고 병반 면적률을 실시간 분석하세요.")

DISEASE_TYPE_NAME = "노란색/갈색 반점" # 노균병에 맞게 변경
DEFAULT_LOWER_DISEASE_HSV = [15, 50, 50]  # 노균병용 기본값 (조정 필요)
DEFAULT_UPPER_DISEASE_HSV = [45, 255, 255] # 노균병용 기본값 (조정 필요)
DEFAULT_LOWER_LEAF_HSV = [35, 40, 40]    # 잎 영역 HSV는 동일하게 사용
DEFAULT_UPPER_LEAF_HSV = [85, 255, 255]   # 잎 영역 HSV는 동일하게 사용

SESSION_KEY_PREFIX = "dm_" # 노균병 페이지 접두사 (Downy Mildew)

uploaded_files = st.file_uploader(
    "노균병 오이 잎 사진 업로드 (여러 장 가능)", # 레이블 변경
    type=["jpg", "jpeg", "png"],
    accept_multiple_files=True,
    key=f"{SESSION_KEY_PREFIX}uploader" # 키 변경
)
...
def get_disease_grade(ratio): # 등급 함수는 동일하게 사용
    if ratio == 0: return 0
...
if uploaded_files:
    ...
    for idx, uploaded_file_obj in enumerate(uploaded_files):
        ...
        if st.session_state[f"{SESSION_KEY_PREFIX}show_cropper"].get(file_specific_key, False):
            ...
            cropped_img_pil = st_cropper(
                pil_bg_image, realtime_update=True, box_color='orange', # 크롭 박스 색상 변경
        ...
        results_for_csv.append({
            "파일명": uploaded_file_obj.name, "질병 종류": "노균병", # 변경
    ...
    if results_for_csv:
        st.markdown("## 📊 전체 결과 요약 및 다운로드 (노균병)") # 변경
        df_results = pd.DataFrame(results_for_csv)
        st.dataframe(df_results)
        csv_output = df_results.to_csv(index=False).encode('utf-8-sig')
        st.download_button(
            label="📥 노균병 결과 저장", data=csv_output, # 변경
            file_name="노균병_분석결과.csv", mime="text/csv", # 변경
            key=f"{SESSION_KEY_PREFIX}download_csv" # 변경
        )

if st.button("🏠 처음으로 돌아가기", key=f"{SESSION_KEY_PREFIX}home_button"): # 변경
